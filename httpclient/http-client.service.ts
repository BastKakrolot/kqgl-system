...
export class HttpClientService {
  ...
  postRequest(url: string, gmParams?: object, gmPayload?: object, options?: GmOptions): Observable<any> {
    // 处理请求头
    const headers = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded;charset=UTF-8');
    // 添加时间戳，避免缓存
    // let timestamp = +new Date();
    // url += (url.indexOf('?') < 0 ? '?' : '&') + `timestamp=${String(timestamp)}`;
    let strPayload = this.httpclientBaseService.paramObject(gmPayload);
    // 处理请求参数
    if (gmParams) {
      let urlfoot = '';
      urlfoot += `&${this.httpclientBaseService.param(gmParams)}`;
      urlfoot = urlfoot.slice(1)
      urlfoot = '?' + urlfoot;
      url += urlfoot;
    }
    const getOptions = options ? Object.assign({ headers }, options) : { headers };
    return this.http.post<any>(url, strPayload, getOptions)
      .pipe(
        retry(2),
        catchError(err => this.handleError(err))
      );
